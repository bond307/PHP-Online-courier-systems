...
include 'lib/db.php';
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if(isset($_POST['submit'])) {
           ...
           $sql = "SELECT * FROM curiar_boy_registration_tbl WHERE curiar_boy_email = '$to_email' ";
            $result  = mysqli_query($conn, $sql);
            if(mysqli_num_rows($result) > 0){
                while($row = mysqli_fetch_assoc($result )){
                    $subject = 'Dhaka City Courier:: Reset your password';
                    $body = '<b>Dear: '.$row['curiar_boy_name'].'</b> <br> Your password is: <b>'.$row['curiar_boy_password'].' </b> ';
